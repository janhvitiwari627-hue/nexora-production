...
if (timings?.assertions?.length) {
  ...
  if (typeof timings.totalDurationMs === "number") {
    lines.push("");
    lines.push(
      `_Total: ${timings.totalDurationMs} ms across ${timings.assertions.length} assertions._`,
    );
  }
  lines.push("");
}
  ...
  try {
    ...
    if (files.length) {
      ...
      for (const f of files) {
        ...
        lines.push(
          artifactUrl ? `- [\`${f}\`](${artifactUrl})${sizeLabel}` : `- \`${f}\`${sizeLabel}`,
        );
      }
      lines.push("");
    }
...
lines.push(
  "# edit .env with your real SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, and SUPABASE_SERVICE_ROLE_KEY",
);
...
lines.push("");
lines.push(
  "The example file is [`scripts/rls.env.example`](https://github.com/" +
    process.env.GITHUB_REPOSITORY +
    "/blob/" +
    (process.env.GITHUB_SHA ?? "main") +
    "/scripts/rls.env.example).",
);
lines.push("");
lines.push(
  "After the run, inspect `$RLS_LOG_DIR/failure-details.json` and `$RLS_LOG_DIR/assertion-timings.json` for the same context shown above.",
);
lines.push("");
lines.push("---");
