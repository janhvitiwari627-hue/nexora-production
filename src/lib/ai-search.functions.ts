...
const ParsedSchema = z.object({
  service: z
    .string()
    .describe("Core service searched, e.g. 'haircut', 'bridal makeup'. Empty string if none."),
  category: z.enum(CATEGORIES).nullable().describe("Best matching salon category, or null."),
  maxBudget: z
    .number()
    .nullable()
    .describe("Max budget in INR if user mentioned 'under X' / 'below X', else null."),
  minRating: z
    .number()
    .min(0)
    .max(5)
    .nullable()
    .describe("Min rating if user said 'top rated', 'best' → 4.5; 'good' → 4.0; else null."),
  area: z
    .string()
    .nullable()
    .describe(
      "Locality / area name extracted, e.g. 'Malviya Nagar'. Null if user said 'near me' or none.",
    ),
  nearMe: z.boolean().describe("True if user implied proximity ('near me', 'nearby', 'closest')."),
  intent: z
    .string()
    .describe("One-line natural summary of what user wants, for the UI to confirm."),
});
