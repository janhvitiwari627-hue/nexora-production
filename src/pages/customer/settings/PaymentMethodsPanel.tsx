...
export function PaymentMethodsPanel() {
  ...
  const initial: PreferredKey = upis.find((u) => u.preferred)
    ? `upi:${upis.find((u) => u.preferred)!.id}`
    : "";
  ...
  return (
    <PanelShell title="Payment methods" subtitle="Manage UPI IDs and saved cards.">
      <div>
        ...
        {addingUpi && (
          <div className="border-border bg-background mb-2 flex items-center gap-2 rounded-xl border p-3">
            ...
            <button
              onClick={addUpi}
              className="bg-primary text-primary-foreground rounded-md px-3 py-2 text-xs font-bold"
            >
              Save
            </button>
            <button
              ...
              className="border-border rounded-md border px-3 py-2 text-xs font-semibold"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="space-y-2">
          {upis.map((u) => {
            ...
            return (
              <div
                key={u.id}
                className="border-border bg-background has-[:checked]:border-primary flex items-center justify-between gap-3 rounded-xl border p-3"
              >
                ...
                <div className="flex items-center gap-1">
                  {isEditing ? (
                    <>
                      <button
                        onClick={() => saveUpiEdit(u.id)}
                        className="text-primary hover:bg-primary/10 rounded-md px-2 py-1 text-xs font-bold"
                      >
                        Save
                      </button>
                      <button
                    ...
                    <>
                      ...
                      <button
                        onClick={() => setUpis((p) => p.filter((x) => x.id !== u.id))}
        ...
        <div className="space-y-2">
          {cards.map((c) => {
            ...
            return (
              <label
                key={c.id}
                className="border-border bg-background has-[:checked]:border-primary flex cursor-pointer items-center justify-between gap-3 rounded-xl border p-3"
              >
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="accent-primary h-4 w-4"
                  />
                  <div className="bg-muted text-muted-foreground grid h-9 w-12 place-items-center rounded-md">
      ...
      <p className="text-muted-foreground mt-4 text-xs">
        Preferred method is used by default at checkout. You can always change it before paying.
      </p>
...
function AddCardModal({
  ...
  return (
    ...
    >
      <div
        className="bg-card border-border w-full max-w-md space-y-3 rounded-2xl border p-5 shadow-2xl"
