          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground mt-0.5 text-xs">
                Hide your profile and pause notifications. You can reactivate by signing in again.
              </p>
            </div>
            <button
              onClick={() => setDeactivateOpen(true)}
              className="rounded-md border border-amber-500/40 px-3 py-1.5 text-xs font-bold text-amber-700 hover:bg-amber-500/10 dark:text-amber-300"
            >
              Deactivate
            </button>
          </div>
        ...
        <div className="border-destructive/30 bg-destructive/5 rounded-xl border p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground mt-0.5 text-xs">
                Permanently remove your account and all associated data after 30 days.
              </p>
            </div>
            <button
              onClick={() => setDeleteOpen(true)}
              className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-md px-3 py-1.5 text-xs font-bold"
            >
              Delete account
            </button>
          </div>
      ...
      {deactivateOpen && (
        <ModalShell title="Deactivate account?" onClose={() => setDeactivateOpen(false)}>
          <p className="text-muted-foreground text-sm">
            Your profile will be hidden and you'll be signed out. Sign in anytime to restore.
          </p>
          <div className="mt-2 flex justify-end gap-2">
            ...
            <button
              onClick={() => setDeactivateOpen(false)}
              className="rounded-md bg-amber-500 px-3 py-2 text-sm font-bold text-white hover:bg-amber-600"
            >
              Deactivate
            </button>
          </div>
    ...
    <ModalShell title="Delete account" onClose={onClose}>
      {step === 1 && (
        <>
          ...
          <button
            onClick={() => setStep(2)}
            className="bg-destructive text-destructive-foreground hover:bg-destructive/90 mt-2 w-full rounded-md py-2 text-sm font-bold"
          >
      ...
      {step === 2 && (
        <>
          <p className="text-muted-foreground text-sm">
            For your security, we've sent a 6-digit OTP to your registered mobile.
          </p>
          ...
          <button
            ...
            onClick={() => setStep(3)}
            className="bg-destructive text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50 mt-2 w-full rounded-md py-2 text-sm font-bold"
          >
            Verify
          </button>
        </>
