  ...
  return (
    <PanelShell title="Privacy" subtitle="Control your visibility and how we use your data.">
      <Field label="Profile visibility">
        <div className="grid gap-2 pt-1 md:grid-cols-3">
          {[
            ...
            <label key={o.v} className="border-border has-[:checked]:border-primary has-[:checked]:bg-primary/5 flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition">
              <input type="radio" name="vis" checked={visibility === o.v} onChange={() => setVisibility(o.v as typeof visibility)} className="accent-primary mt-0.5 h-4 w-4" />
              <div>
      ...
      <div className="mt-6 space-y-3">
        <ToggleRow label="Show my activity" desc="Reviews and visits appear on your public profile." checked={activity} onChange={setActivity} />
        <ToggleRow label="AI recommendations" desc="Use my booking history to personalise shop suggestions." checked={aiReco} onChange={setAiReco} />
      </div>

      <div className="mt-6">
        <h3 className="text-heading mb-2 text-xs font-black uppercase tracking-wide">Data consent</h3>
        <div className="space-y-2">
          {[
            ...
            <label key={o.k} className="border-border bg-background flex cursor-pointer items-start gap-3 rounded-xl border p-3 text-sm">
              <input
                type="checkbox"
