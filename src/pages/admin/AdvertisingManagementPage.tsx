...
type Campaign = { id: string; name: string; type: string; budget: number; impressions: number; clicks: number; status: string };

const seedCampaigns = (type: string): Campaign[] => Array.from({ length: 5 }).map((_, i) => ({
  id: `${type}-${i}`,
  name: `${type === "listing" ? "Listing" : "Video"} Campaign ${i + 1}`,
  type, budget: 5000 + i * 1500,
...
export function AdvertisingManagementPage() {
  ...
  const CampaignTable = ({ data }: { data: Campaign[] }) => (
    <Table>
      <TableHeader><TableRow><TableHead>Campaign</TableHead><TableHead>Budget</TableHead><TableHead>Impr.</TableHead><TableHead>Clicks</TableHead><TableHead>CTR</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
      <TableBody>{data.map(c => {
        ...
        return (
          <TableRow key={c.id}>
            ...
            <TableCell>
              <span className={`inline-flex items-center gap-1 ${ctr >= 3 ? "text-emerald-600" : "text-amber-600"}`}>
                {ctr >= 3 ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
                {ctr.toFixed(2)}%
              </span>
            </TableCell>
  ...
  const CreateForm = ({ kind, onAdd }: { kind: "listing" | "video"; onAdd: (c: Campaign) => void }) => {
    const [form, setForm] = useState({ name: "", budget: "", placement: "homepage", notes: "" });
    return (
      <Card>
        <CardHeader><CardTitle className="text-base">Create {kind === "listing" ? "Sponsored Listing" : "Sponsored Video"}</CardTitle></CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-2">
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button onClick={() => {
              ...
              onAdd({ id: `${kind}-${Date.now()}`, name: form.name, type: kind, budget: Number(form.budget), impressions: 0, clicks: 0, status: "Active" });
              setForm({ name: "", budget: "", placement: "homepage", notes: "" });
              toast.success("Campaign created");
            }}><Plus className="h-4 w-4" /> Create Campaign</Button>
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      ...
      <Tabs defaultValue="listings">
        ...
        <TabsContent value="listings" className="space-y-4">
          ...
          <Card><CardContent className="p-0"><CampaignTable data={listings} /></CardContent></Card>
        </TabsContent>

        <TabsContent value="videos" className="space-y-4">
          <CreateForm kind="video" onAdd={c => setVideos(p => [c, ...p])} />
          <Card><CardContent className="p-0"><CampaignTable data={videos} /></CardContent></Card>
        </TabsContent>

        <TabsContent value="perf">
          <Card><CardContent className="p-0"><CampaignTable data={[...listings, ...videos]} /></CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
