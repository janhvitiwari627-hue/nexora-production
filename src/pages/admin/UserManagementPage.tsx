...
const ROLE_FILTERS = ["all", "customer", "owner", "shop_owner", "admin", "super_admin", "growth_partner", "district_partner", "brand", "distributor"] as const;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <header>
        ...
        <p className="text-muted-foreground text-sm">All platform users — real-time from database.</p>
      </header>
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-3 p-4">
          ...
          <Select value={role} onValueChange={setRole}>
            ...
            <SelectContent>
              {ROLE_FILTERS.map((r) => (
                <SelectItem key={r} value={r} className="capitalize">{r === "all" ? "All Roles" : r.replace("_", " ")}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          <div className="text-muted-foreground text-sm">{filtered.length} of {users.length}</div>
        </CardContent>
      </Card>
              ...
              <TableBody>
                {filtered.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell>
                      <Badge variant={u.is_active === false ? "secondary" : "default"} className="capitalize">
            ...
            <>
              ...
              <div className="mt-4 space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><div className="text-muted-foreground text-xs">City</div>{detail.city ?? "—"}</div>
                  <div><div className="text-muted-foreground text-xs">Verified</div>{detail.is_verified ? "Yes" : "No"}</div>
                  <div><div className="text-muted-foreground text-xs">Joined</div>{new Date(detail.created_at).toLocaleString()}</div>
                  <div><div className="text-muted-foreground text-xs">Status</div>{detail.is_active === false ? "Suspended" : "Active"}</div>
                </div>
                <div>
                  <div className="text-muted-foreground text-xs">Roles</div>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {detail.roles.length === 0 ? <Badge variant="outline">no role</Badge> :
                      detail.roles.map((r) => <Badge key={r} variant="outline" className="capitalize">{r.replace("_", " ")}</Badge>)}
                  </div>
                </div>
                <div className="flex gap-2 pt-2">
                  {detail.is_active === false ? (
                    <Button className="flex-1" onClick={() => suspend.mutate({ id: detail.id, active: true })} disabled={suspend.isPending}>
                      Reactivate
                    </Button>
                  ) : (
                    <Button variant="destructive" className="flex-1" onClick={() => suspend.mutate({ id: detail.id, active: false })} disabled={suspend.isPending}>
                      Suspend
                    </Button>
                  )}
                </div>
