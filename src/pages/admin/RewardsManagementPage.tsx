      ...
      <Card>
        ...
        <CardContent>
              ...
              <div key={r.id} className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      setRules((p) =>
                        p.map((x) => (x.id === r.id ? { ...x, value: Number(e.target.value) } : x)),
                      )
                    }
                  />
                  <span className="bg-muted grid w-16 place-items-center rounded-md text-sm">
                    {r.unit}
                  </span>
                </div>
              </div>
          ...
          <div className="mt-4 flex justify-end">
            <Button onClick={() => toast.success("Rules saved")}>Save Rules</Button>
          </div>
        </CardContent>
      </Card>
          ...
          {selected.size > 0 && (
            <div className="flex gap-2">
              <Button
                ...
                variant="outline"
                onClick={() => {
                  toast.success(`Reversed ${selected.size} entries`);
                  setSelected(new Set());
                }}
              >
                Bulk Reverse
              ...
              <Button
                ...
                variant="destructive"
                onClick={() => {
                  toast.success(`Voided ${selected.size}`);
                  setSelected(new Set());
                }}
              >
                Bulk Void
            ...
            <TableBody>
              {AUDIT.map((a) => (
                <TableRow key={a.id} className={a.flagged ? "border-l-4 border-l-destructive" : ""}>
                  ...
                  <TableCell className={a.points < 0 ? "text-destructive" : "text-emerald-600"}>
                    {a.points > 0 ? "+" : ""}
                    {a.points}
                  </TableCell>
