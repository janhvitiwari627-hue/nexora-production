...
const AREAS = [
  "Mansarovar", "Vaishali Nagar", "Malviya Nagar", "Raja Park",
  "Jagatpura", "Tonk Road", "Sodala", "Jhotwara", "Sanganer",
];

const QUICK_FILTERS = [
  "Near Me", "Open Now", "Top Rated", "Price Low to High",
  "Male", "Female", "Unisex",
];

const CATEGORIES = [
  { name: "Salon", desc: "Haircuts, styling & color experts", Icon: Scissors, gradient: "from-blue-500 to-indigo-600" },
  { name: "Beauty Parlour", desc: "Facials, threading & care", Icon: Flower2, gradient: "from-pink-500 to-rose-600" },
  { name: "Spa", desc: "Relax, recharge, rejuvenate", Icon: HeartPulse, gradient: "from-emerald-500 to-teal-600" },
  { name: "Tattoo Studio", desc: "Verified artists & hygiene", Icon: Brush, gradient: "from-slate-700 to-slate-900" },
  { name: "Massage Center", desc: "Therapeutic & wellness", Icon: Hand, gradient: "from-amber-500 to-orange-600" },
  { name: "Nail Art Studio", desc: "Premium nails & extensions", Icon: PaintBucket, gradient: "from-fuchsia-500 to-purple-600" },
  { name: "Makeup Artist", desc: "Pro looks for every occasion", Icon: Palette, gradient: "from-violet-500 to-indigo-600" },
  { name: "Bridal Services", desc: "Luxury bridal packages", Icon: Crown, gradient: "from-yellow-500 to-amber-600" },
];
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1400px] items-center justify-between px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-9 md:flex">
          {links.map((l) => (
            <Link key={l.label} to={l.href} className="text-sm font-medium text-slate-600 transition hover:text-slate-900">
              {l.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button className="hidden h-10 w-10 place-items-center rounded-full text-slate-600 hover:bg-slate-100 sm:grid" aria-label="Search">
      ...
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 items-center gap-14 px-5 pb-20 pt-16 sm:px-8 lg:grid-cols-[1.05fr_1fr] lg:gap-10 lg:pb-28 lg:pt-24">
        ...
        <div>
          ...
          <h1 className="mt-6 font-bold tracking-tight text-slate-900 text-[34px] leading-[1.05] sm:text-[44px] lg:text-[64px]">
            Book Jaipur's <span className="bg-gradient-to-r from-[#2563EB] via-[#4F46E5] to-[#6D28D9] bg-clip-text text-transparent">Trusted Beauty</span> Services in 60 Seconds
          </h1>

          <p className="mt-6 max-w-xl text-[16px] leading-relaxed text-slate-600 sm:text-[18px]">
            Search verified salons, beauty parlours, spas, tattoo studios, massage centers and nail art studios near you.
          </p>
...
function HeroVisual() {
  return (
    <div className="relative mx-auto h-[520px] w-full max-w-[560px] lg:h-[600px]">
      ...
      <svg aria-hidden viewBox="0 0 400 220" className="absolute -bottom-6 left-1/2 -z-10 w-[110%] -translate-x-1/2 opacity-[0.07]">
        <path d="M0 220 V120 Q40 80 80 120 Q120 60 160 120 Q200 50 240 120 Q280 60 320 120 Q360 80 400 120 V220 Z" fill="#0F172A"/>
      </svg>
      ...
      <div className="absolute left-0 top-0 w-[78%] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_30px_80px_-20px_rgba(15,23,42,0.25)]">
        <div className="relative h-[200px] overflow-hidden">
          <img alt="" src="https://images.unsplash.com/photo-1560066984-138dadb4c035?w=1200&q=80" className="h-full w-full object-cover" />
        ...
        <div className="p-5">
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xs text-slate-500">From <span className="font-bold text-slate-900">₹499</span></p>
            <span className="rounded-full bg-slate-900 px-3 py-1.5 text-[11px] font-semibold text-white">View</span>
          </div>
        </div>
      </div>
      ...
      <div className="absolute right-[4%] bottom-[40px] w-[48%] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="relative h-[90px] bg-gradient-to-br from-blue-50 to-violet-50">
          <div className="absolute inset-0 opacity-50" style={{
            backgroundImage: "radial-gradient(circle at 20% 30%, rgba(37,99,235,.25) 1px, transparent 1px), radial-gradient(circle at 70% 60%, rgba(109,40,217,.2) 1px, transparent 1px)",
      ...
      <div className="absolute left-[2%] bottom-0 flex w-[44%] items-center gap-3 rounded-2xl border border-slate-200 bg-white p-3.5 shadow-xl">
        <div className="flex -space-x-2">
          {["a","b","c"].map((k, i) => (
            <div key={k} className={`h-8 w-8 rounded-full border-2 border-white bg-gradient-to-br ${
              i === 0 ? "from-pink-400 to-rose-500" : i === 1 ? "from-blue-400 to-indigo-500" : "from-amber-400 to-orange-500"
            }`} />
          ))}
        </div>
        <div>
    ...
    <div className="mx-auto -mt-12 max-w-[1280px] px-5 sm:-mt-16 sm:px-8 lg:-mt-20">
      ...
      >
        <div className="grid grid-cols-1 gap-2 sm:gap-3 lg:grid-cols-[1.4fr_1fr_1fr_auto]">
          <Field label="What are you looking for?" icon={<Search className="h-4 w-4 text-slate-400" />}>
          ...
          <Field label="Category" icon={<Layers className="h-4 w-4 text-slate-400" />}>
            <select value={cat} onChange={(e) => setCat(e.target.value)}
              ...
              {["Salon", "Spa", "Beauty Parlour", "Tattoo Studio", "Massage Center", "Nail Art Studio"].map((c) =>
      ...
      <div className="mt-3 flex flex-wrap gap-2.5">
        {AREAS.map((a) => (
          <Link key={a} to="/search" search={{ area: a } as never}
...
function Field({ label, icon, children }: { label: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <label className="group flex h-[72px] items-center gap-3 rounded-2xl border border-transparent bg-slate-50 px-5 transition hover:border-slate-200 hover:bg-white">
      <span className="grid h-9 w-9 flex-none place-items-center rounded-xl bg-white shadow-sm">{icon}</span>
...
function CategoryGrid() {
  return (
    <Section eyebrow="Categories" title="Discover by category" subtitle="Premium tiles for every beauty experience in Jaipur.">
      <div className="grid grid-cols-2 gap-4 sm:gap-5 lg:grid-cols-4">
        {CATEGORIES.map(({ name, desc, Icon, gradient }) => (
          ...
          >
            ...
            <div aria-hidden className={`pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-gradient-to-br ${gradient} opacity-0 blur-2xl transition group-hover:opacity-20`} />
          </Link>
        ))}
      </div>
    </Section>
...
function ShopListings() {
  ...
  return (
    <Section
      ...
      action={<Link to="/search" className="inline-flex items-center gap-1 text-sm font-semibold text-slate-900">View all <ArrowRight className="h-4 w-4" /></Link>}
    >
      {isLoading ? (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-[440px] animate-pulse rounded-[28px] border border-slate-200 bg-white" />
          ))}
        </div>
      ) : shops.length === 0 ? (
        <EmptyState />
  ...
  return (
    <article className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white transition hover:-translate-y-1 hover:shadow-2xl">
      <div className="relative h-[240px] overflow-hidden">
        <img
          src={shop.cover_image ?? "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=1200&q=80"}
          alt={shop.name}
          loading="lazy"
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-[13px] text-slate-500">
              {shop.category ?? "Salon"} · {shop.area ?? ""}{shop.city ? `, ${shop.city}` : ""}
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {["Haircut", "Facial", "Spa"].map((t) => (
            <span key={t} className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-700">{t}</span>
          ))}
        </div>
        <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
          <p className="text-[12px] text-slate-500">Starting at <span className="text-[15px] font-bold text-slate-900">{price}</span></p>
          <div className="flex gap-2">
            ...
            <Link to="/shop/$slug" params={{ slug: shop.slug }} search={{ book: "1" } as never}
...
function EmptyState() {
  return (
    <div className="rounded-[32px] border border-dashed border-slate-300 bg-white p-12 text-center">
      ...
      <p className="mt-1 text-sm text-slate-500">Be the first salon to list and reach Jaipur customers.</p>
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <Link to="/owner/create-website" className="inline-flex h-11 items-center rounded-full bg-slate-900 px-5 text-sm font-semibold text-white">Create Shop Website</Link>
        <Link to="/admin/login" className="inline-flex h-11 items-center rounded-full border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-800">Admin: Add Demo Shop</Link>
      </div>
    </div>
  );
}
...
function SmartPicks() {
  ...
  const picks = useMemo(() => {
    ...
    list.sort((a, b) =>
      (Number(b.rating ?? 0) * 10 + Number(b.review_count ?? 0) / 100) -
  ...
  return (
    <Section
      eyebrow={<span className="inline-flex items-center gap-1.5"><TrendingUp className="h-3.5 w-3.5" /> AI Picks</span>}
...
function WebsiteBuilder() {
  const templates = [
    { name: "Royal Luxe", tag: "Luxury · Black & Gold", img: "https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=1200&q=80", gradient: "from-amber-200 via-yellow-100 to-rose-100" },
    { name: "Modern Salon", tag: "Bold · Red & White", img: "https://images.unsplash.com/photo-1560066984-138dadb4c035?w=1200&q=80", gradient: "from-rose-100 via-white to-slate-100" },
    { name: "Professional Beauty", tag: "Soft · Pink & Rose Gold", img: "https://images.unsplash.com/photo-1487412947147-5cebf100ffc2?w=1200&q=80", gradient: "from-pink-100 via-rose-50 to-white" },
  ];
  return (
    <Section
      ...
      action={<Link to="/owner/create-website" className="inline-flex items-center gap-1 text-sm font-semibold text-slate-900">Browse all <ArrowRight className="h-4 w-4" /></Link>}
    >
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {templates.map((t) => (
          <div key={t.name} className="group overflow-hidden rounded-[28px] border border-slate-200 bg-white transition hover:-translate-y-1 hover:shadow-2xl">
            <div className={`relative h-[220px] overflow-hidden bg-gradient-to-br ${t.gradient}`}>
              <img src={t.img} alt={t.name} loading="lazy" className="h-full w-full object-cover mix-blend-multiply opacity-90 transition duration-700 group-hover:scale-105" />
            </div>
            <div className="p-6">
              <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{t.tag}</p>
              <p className="mt-1 text-[20px] font-bold text-slate-900">{t.name}</p>
              <p className="mt-1 text-sm text-slate-500">Premium pages, booking, gallery, reviews & more.</p>
              <Link to="/owner/create-website" className="mt-5 inline-flex h-11 items-center gap-2 rounded-full bg-slate-900 px-5 text-sm font-semibold text-white">
      ...
      <div className="mt-10 flex justify-center">
        <Link to="/owner/create-website" className="inline-flex h-12 items-center gap-2 rounded-full bg-gradient-to-r from-[#2563EB] to-[#6D28D9] px-7 text-sm font-bold text-white shadow-lg shadow-blue-200/70">
...
function MembershipCards() {
  const tiers = [
    { name: "Silver", price: "₹299/mo", perks: ["5% off bookings", "Priority slots", "Member offers"], grad: "from-slate-200 via-slate-100 to-slate-300", text: "text-slate-900" },
    { name: "Gold", price: "₹599/mo", perks: ["10% off bookings", "Free birthday spa", "Exclusive lounges"], grad: "from-amber-300 via-yellow-200 to-amber-400", text: "text-amber-950" },
    { name: "Platinum", price: "₹1,299/mo", perks: ["15% off + cashbacks", "Premium concierge", "Bridal early access"], grad: "from-slate-700 via-slate-800 to-black", text: "text-white" },
  ];
  return (
    <Section eyebrow="Membership" title="Nexora Membership" subtitle="Premium perks built for India's beauty industry.">
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {tiers.map((t) => (
          <div key={t.name} className={`relative overflow-hidden rounded-[28px] border border-slate-200 p-7 shadow-xl bg-gradient-to-br ${t.grad} ${t.text}`}>
            <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-white/30 to-transparent" />
            <div className="flex items-center justify-between">
              <p className="text-[12px] font-bold uppercase tracking-[0.2em] opacity-80">{t.name}</p>
            ...
            <button className={`mt-8 inline-flex h-11 w-full items-center justify-center rounded-full text-[13px] font-bold ${
  ...
  return (
    <section className="px-5 py-20 sm:px-8 lg:py-28">
      <div className="relative mx-auto max-w-[1280px] overflow-hidden rounded-[40px] bg-gradient-to-br from-slate-900 via-[#1E1B4B] to-[#3B0764] p-10 text-center text-white sm:p-16">
        <div aria-hidden className="pointer-events-none absolute -left-20 -top-20 h-72 w-72 rounded-full bg-blue-500/20 blur-3xl" />
        <div aria-hidden className="pointer-events-none absolute -right-20 -bottom-20 h-72 w-72 rounded-full bg-fuchsia-500/20 blur-3xl" />
        <h2 className="mx-auto max-w-3xl text-[32px] font-bold tracking-tight sm:text-[48px]">
          Salon Ja Rahe Ho? <span className="bg-gradient-to-r from-blue-300 to-fuchsia-300 bg-clip-text text-transparent">Nexora Kiya Kya?</span>
        </h2>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Link to="/search" className="inline-flex h-12 items-center gap-2 rounded-full bg-white px-6 text-sm font-bold text-slate-900">
            Explore Salons <ArrowRight className="h-4 w-4" />
          </Link>
          <Link to="/owner/create-website" className="inline-flex h-12 items-center gap-2 rounded-full border border-white/30 bg-white/5 px-6 text-sm font-bold text-white backdrop-blur">
            Create Shop Website
          </Link>
        </div>
      </div>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 backdrop-blur md:hidden">
      <div className="grid grid-cols-4">
        {items.map(({ label, to, Icon }) => (
          <Link key={label} to={to} className="flex flex-col items-center gap-1 py-2.5 text-[11px] font-semibold text-slate-600">
...
function Section({
  ...
  eyebrow?: React.ReactNode; title: string; subtitle?: string;
      ...
      <div className="mx-auto max-w-[1400px]">
        <div className="mb-10 flex flex-col items-start justify-between gap-4 sm:mb-12 sm:flex-row sm:items-end">
          <div className="max-w-2xl">
            ...
            <h2 className="text-[28px] font-bold tracking-tight text-slate-900 sm:text-[40px]">{title}</h2>
            {subtitle && <p className="mt-3 text-[15px] text-slate-600 sm:text-[17px]">{subtitle}</p>}
          </div>
          {action}
        </div>
        {children}
