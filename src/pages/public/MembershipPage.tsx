...
const PLANS = [
  {
    ...
    price: 999,
    period: "year",
    color: "from-slate-200 to-slate-400 text-slate-900",
    featured: false,
    perks: ["10% off all services", "Member-only offers", "Priority customer support"],
  },
  {
    name: "Gold",
    price: 2499,
    period: "year",
    color: "from-amber-200 via-yellow-300 to-amber-500 text-amber-950",
    featured: true,
    perks: [
      "15% off all services",
      "Priority booking",
  ...
  },
  {
    ...
    price: 4999,
    period: "year",
    color: "from-indigo-900 via-indigo-700 to-violet-500 text-white",
    featured: false,
    perks: [
      "25% off all services",
      "VIP slots",
...
const TESTIMONIALS = [
  {
    name: "Riya Bhatia",
    tier: "Gold",
    text: "I've saved over ₹12,000 this year on services I'd have booked anyway. No-brainer.",
  },
  {
    name: "Karan Malhotra",
    tier: "Platinum",
    text: "Priority slots on Saturdays are gold. Plus the monthly facial pays for the membership in 4 months.",
  },
  {
    name: "Sneha Reddy",
    tier: "Silver",
    text: "Started with Silver to try it. Already planning to upgrade — the offers keep getting better.",
  },
];
...
const FAQS = [
  {
    ...
    a: "Immediately upon purchase. You'll get a digital Nexora ID instantly.",
  },
  {
    q: "Can I use my membership at all locations?",
    a: "Yes — membership is honoured at every partner across India.",
  },
  { q: "Can I cancel anytime?", a: "Yes. Prorated refund available within the first 30 days." },
  {
    q: "Can I gift a membership?",
    a: "Absolutely. Choose 'Gift' at checkout and we'll send a beautiful e-card.",
  },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="from-primary/10 to-accent/10 border-border relative overflow-hidden border-b bg-gradient-to-br py-20 text-center md:py-28">
        ...
        <div className="relative mx-auto max-w-3xl px-4">
          ...
          <h1
            className="text-heading mt-4 text-4xl font-black tracking-tight md:text-6xl"
            style={{ fontFamily: "Inter, sans-serif" }}
          >
            Save more. Style more.
          </h1>
          <p className="text-muted-foreground mt-4 text-base md:text-lg">
            Members save an average of ₹14,000/year on services they'd book anyway. Join 38,000+
            happy members.
          </p>
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {PLANS.map((p) => (
            <motion.article
              ...
              whileHover={{ y: -4 }}
              className={cn(
                "relative overflow-hidden rounded-[24px] p-7 shadow-[var(--shadow-card)]",
                p.featured && "ring-4 ring-primary/30",
              )}
            >
              <div className={cn("absolute inset-0 bg-gradient-to-br opacity-100", p.color)} />
              <div className="relative">
                {p.featured && (
                  <span className="bg-card text-primary mb-3 inline-block rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-wider">
                    Most Popular
                  </span>
                )}
                ...
                <div className="mt-3 flex items-baseline gap-1">
                  <span className="text-3xl font-black sm:text-4xl">
                    ₹{billing === "month" ? Math.round(p.price / 10) : p.price}
                  </span>
            ...
            <table className="w-full text-sm">
              <thead className="border-border border-b bg-muted/50">
                <tr className="text-heading">
                  ...
                  {PLANS.map((p) => (
                    <th key={p.name} className="p-4 text-center font-bold">
                      {p.name}
                    </th>
                  ))}
                </tr>
                    ...
                    {row.slice(1).map((v, j) => (
                      <td key={j} className="p-4 text-center">
                        {v === true ? (
                          ...
                          <span className="text-heading font-bold">{v as string}</span>
                        )}
                      </td>
                    ))}
                  </tr>
