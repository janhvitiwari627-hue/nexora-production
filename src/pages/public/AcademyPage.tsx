...
const COURSES = [
  ...
  {
    id: "c3",
    title: "Aromatherapy Foundations",
    level: "Beginner",
    duration: "4 weeks",
    price: 2999,
    rating: 4.7,
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#0A2540] via-[#1a1060] to-[#635BFF] py-20 md:py-28">
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center md:px-6">
          ...
          <p className="mt-4 text-base text-white/85 md:text-lg">
            Industry-recognised certifications taught by award-winning professionals. Built for
            stylists, by stylists.
          </p>
        </div>
      </section>
        ...
        <div className="mt-10 grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          {CATEGORIES.map((c) => (
            <motion.button
              key={c.name}
              whileHover={{ y: -3 }}
              className={`overflow-hidden rounded-[20px] bg-gradient-to-br p-5 text-left shadow-[var(--shadow-card)] ${c.color}`}
            >
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {COURSES.map((c) => (
              <article
                key={c.id}
                className="border-border bg-card flex flex-col overflow-hidden rounded-[var(--radius-card)] border transition hover:-translate-y-1 hover:shadow-[var(--shadow-glow)]"
              >
                ...
                <div className="flex flex-1 flex-col p-4">
                  ...
                  <div className="text-muted-foreground mt-3 flex items-center gap-3 text-xs">
                    ...
                    <span className="inline-flex items-center gap-1">
                      <Star className="text-warning fill-warning h-3 w-3" /> {c.rating}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <Users className="h-3 w-3" /> {c.students.toLocaleString("en-IN")}
                    </span>
                  </div>
                  <div className="border-border mt-4 flex items-center justify-between border-t pt-3">
                    <span className="text-heading text-lg font-black">
                      ₹{c.price.toLocaleString("en-IN")}
                    </span>
                    <button className="bg-gradient-cta text-primary-foreground rounded-[var(--radius-button)] px-3 py-1.5 text-xs font-bold">
                      Enrol
                    </button>
                  </div>
                </div>
      ...
      <section className="mx-auto max-w-5xl px-4 py-20 md:px-6">
        <div className="from-primary/10 to-accent/10 border-primary/30 grid items-center gap-8 rounded-[24px] border bg-gradient-to-br p-8 md:grid-cols-[1fr_280px] md:p-12">
          <div>
            ...
            <h3 className="text-heading mt-4 text-3xl font-black md:text-4xl">
              Industry-recognised certificates
            </h3>
            <p className="text-muted-foreground mt-3">
              Every course ends with a Nexora Academy certificate, recognised by 12,000+ partner
              salons. Get hired faster, earn more.
            </p>
            <ul className="text-heading mt-5 grid gap-2 text-sm md:grid-cols-2">
              {[
                "Lifetime access",
                "Hands-on practice kits",
                "Live Q&A sessions",
                "Job placement help",
              ].map((b) => (
                <li key={b} className="flex items-start gap-2">
                  <BadgeCheck className="text-primary mt-0.5 h-4 w-4 shrink-0" /> {b}
                </li>
              ))}
            </ul>
          </div>
          <div className="border-primary/30 from-card to-muted relative aspect-[4/3] overflow-hidden rounded-[20px] border bg-gradient-to-br p-6 text-center shadow-xl">
            ...
            <div className="text-muted-foreground mt-2 text-[10px] uppercase tracking-wider">
              Certified
            </div>
        ...
        <div className="mx-auto max-w-6xl px-4 md:px-6">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            {INSTRUCTORS.map((i) => (
              <article
                key={i.name}
                className="border-border bg-card rounded-[var(--radius-card)] border p-5 text-center"
              >
                <img
                  src={i.avatar}
                  alt={i.name}
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 md:px-6">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {[
            {
              name: "Meera Kapoor",
              text: "I doubled my salary in 6 months after finishing the Bridal Makeup course.",
            },
            {
              name: "Arjun Verma",
              text: "Loved the hands-on kits. Felt like learning from a friend, not a textbook.",
            },
            {
              name: "Sneha Reddy",
              text: "Got placed in a top salon within 2 weeks of getting certified.",
            },
          ].map((t) => (
            <article
              key={t.name}
            ...
            >
              ...
              <div className="text-heading border-border mt-4 border-t pt-3 text-sm font-bold">
                {t.name}
              </div>
            </article>
          ))}
