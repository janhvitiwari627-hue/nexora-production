  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="mx-auto grid max-w-5xl gap-8 px-4 py-16 md:grid-cols-[1fr_320px] md:px-6">
        <form
          onSubmit={submit}
          className="border-border bg-card rounded-[var(--radius-card-lg)] border p-7 shadow-[var(--shadow-card)]"
        >
          {sent ? (
            <div className="bg-success/10 text-success rounded-[var(--radius-card)] p-8 text-center font-bold">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {(
                ...
                <label key={k} className={full ? "md:col-span-2" : ""}>
                  <span className="text-heading text-xs font-bold uppercase tracking-wider">
                    {l}
                  </span>
                  <input
                    value={form[k]}
                  ...
                  />
                  {errors[k] && (
                    <span className="text-danger mt-1 block text-xs font-semibold">
                      {errors[k]}
                    </span>
                  )}
                </label>
              ...
              <button
                type="submit"
                className="bg-gradient-cta text-primary-foreground md:col-span-2 mt-2 rounded-[var(--radius-button)] px-4 py-3 text-sm font-bold shadow-[var(--shadow-glow)]"
              >
                Send message
              </button>
            </div>
