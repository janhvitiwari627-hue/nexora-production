...
const STEPS = [
  {
    ...
    body: "Send your unique referral code to friends via WhatsApp or social.",
  },
  {
    icon: UserPlus,
    title: "Friend signs up",
    body: "They get ₹200 off their first booking — a sweet welcome.",
  },
  {
    icon: Coins,
    title: "You earn rewards",
    body: "Get 500 reward points after their first completed booking.",
  },
];
          ...
          {STEPS.map((s, i) => (
            ...
            >
              <div className="border-border bg-card rounded-[var(--radius-card-lg)] border p-7 text-center shadow-[var(--shadow-card)]">
                ...
                <div className="text-muted-foreground mt-3 text-[10px] font-black uppercase tracking-wider">
                  Step {i + 1}
                </div>
              ...
              {i < STEPS.length - 1 && (
                <motion.svg
                  viewBox="0 0 100 20"
                  className="text-primary hidden md:block absolute top-1/2 -right-6 h-6 w-12 -translate-y-1/2"
                ...
                >
                  <motion.path
                    d="M5 10 H85 M75 4 L88 10 L75 16"
                    stroke="currentColor"
                    strokeWidth="2.5"
                    fill="none"
                    strokeLinecap="round"
      ...
      <section className="mx-auto max-w-3xl px-4 pb-16 md:px-6">
        {isLoggedIn ? (
          <div className="from-primary/10 via-card to-accent/10 border-primary/30 rounded-[24px] border bg-gradient-to-br p-8 text-center shadow-[var(--shadow-glow)]">
            ...
            <div className="text-heading mt-2 font-mono text-3xl font-black tracking-widest md:text-4xl">
              {code}
            </div>
            <div className="mt-5 flex flex-wrap justify-center gap-2">
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-1.5 rounded-[var(--radius-button)] px-5 py-2.5 text-sm font-bold transition",
                  copied ? "bg-success text-white" : "border-primary text-primary border-2",
                )}
              >
              ...
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Join me on Nexora — use my code ${code} for ₹200 off your first booking!`)}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1.5 rounded-[var(--radius-button)] bg-[#25D366] px-5 py-2.5 text-sm font-bold text-white"
              >
          ...
          <div className="border-border bg-card rounded-[24px] border p-8 text-center">
            ...
            <p className="text-muted-foreground mt-2 text-sm">
              Log in or create an account to generate your unique referral link.
            </p>
            <button className="bg-gradient-cta text-primary-foreground mt-5 rounded-[var(--radius-button)] px-6 py-3 text-sm font-bold shadow-[var(--shadow-glow)]">
              Sign in to continue
            </button>
          </div>
        )}
      ...
      <section className="bg-muted/30 border-t border-border py-16">
        <div className="mx-auto max-w-3xl px-4 md:px-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-heading text-2xl font-black md:text-3xl">
