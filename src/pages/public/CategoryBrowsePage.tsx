...
const CATEGORIES: Record<string, { name: string; tagline: string; gradient: string; subs: string[] }> = {
  ...
  "beauty-parlour": { name: "Beauty Parlours", tagline: "Facials, threading, waxing — your beauty ritual", gradient: "from-[#EC4899] via-[#F97316] to-[#F59E0B]", subs: ["Facial", "Bridal", "Threading", "Waxing"] },
  spa: { name: "Spa & Wellness", tagline: "Relax, unwind, restore", gradient: "from-[#0EA5E9] via-[#6366F1] to-[#8B5CF6]", subs: ["Aromatherapy", "Couple Spa", "Foot Spa", "Body Polish"] },
  tattoo: { name: "Tattoo Studios", tagline: "Tell your story in ink", gradient: "from-[#0F172A] via-[#1E293B] to-[#635BFF]", subs: ["Fine line", "Black & grey", "Colour", "Cover-up"] },
  massage: { name: "Massage Centres", tagline: "Knots out. Calm in.", gradient: "from-[#10B981] via-[#0EA5E9] to-[#6366F1]", subs: ["Swedish", "Deep tissue", "Thai", "Ayurvedic"] },
  nails: { name: "Nail Studios", tagline: "Polished to perfection", gradient: "from-[#F472B6] via-[#FB7185] to-[#F59E0B]", subs: ["Gel", "Acrylic", "Nail art", "Extensions"] },
  makeup: { name: "Makeup Artists", tagline: "From everyday to occasion", gradient: "from-[#F43F5E] via-[#EC4899] to-[#8B5CF6]", subs: ["Party", "Bridal", "HD", "Airbrush"] },
  mehendi: { name: "Mehendi Artists", tagline: "Tradition, designed beautifully", gradient: "from-[#F59E0B] via-[#F97316] to-[#EF4444]", subs: ["Bridal", "Arabic", "Indo-Arabic", "Minimal"] },
};

const AREAS = ["All Jaipur", "Malviya Nagar", "C-Scheme", "Vaishali Nagar", "Mansarovar", "Raja Park", "Jagatpura", "Tonk Road"];
const SORTS = ["Recommended", "Top rated", "Nearest", "Most booked"];

const SAMPLE_SHOPS: Shop[] = Array.from({ length: 8 }).map((_, i) => ({
  slug: `sample-${i}`,
  name: ["Looks Unisex Salon", "Bliss Spa", "The Barber Co.", "Glow Studio", "Halo Hair Lounge", "Studio Noir", "Pure Skin Clinic", "Nail Boutique"][i],
    ...
    <div className="min-h-screen bg-background">
      <section className={cn("relative overflow-hidden bg-gradient-to-br py-16 md:py-24", cat.gradient)}>
        ...
        <div className="relative mx-auto max-w-7xl px-4 md:px-6">
          ...
          <h1 className="mt-4 text-4xl font-black tracking-tight text-white md:text-6xl" style={{ fontFamily: "Inter, sans-serif" }}>{cat.name}</h1>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:px-6 lg:grid-cols-[1fr_300px]">
        <main>
          ...
          <div className="border-border bg-card mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-card)] border p-3">
            <select value={area} onChange={(e) => setArea(e.target.value)} className="bg-background border-border rounded-[var(--radius-button)] border px-3 py-2 text-sm font-semibold">
              {AREAS.map((a) => <option key={a}>{a}</option>)}
            </select>
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="bg-background border-border rounded-[var(--radius-button)] border px-3 py-2 text-sm font-semibold">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map((s) => <ShopCard key={s.slug} shop={s} variant="grid" />)}
          </div>
        </main>

        <aside className="space-y-4">
          <InsightCard title="Trending in your area" lines={["Bridal packages — bookings up 38% this month", "Hair colour services peak on Saturdays", "Best value: morning slots before 11 AM"]} />
          <InsightCard title="Popular searches" lines={cat.subs.map((s) => `${s} near me`)} />
          <Link to="/membership" className="from-primary/10 to-accent/10 border-primary/30 block rounded-[var(--radius-card)] border bg-gradient-to-br p-5">
            <Sparkles className="text-primary h-5 w-5" />
            <h4 className="text-heading mt-2 text-base font-bold">Save up to 25%</h4>
            <p className="text-muted-foreground mt-1 text-xs">Join Nexora Membership for member-only pricing</p>
            <span className="text-primary mt-3 inline-flex items-center text-sm font-bold">Learn more <ChevronRight className="h-4 w-4" /></span>
          </Link>
        </aside>
      </div>
    </div>
...
function Stat({ icon: Icon, value, label }: { icon: any; value: string; label: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1 backdrop-blur">
      <Icon className="h-3.5 w-3.5" /><b>{value}</b> {label}
    </span>
  );
}
