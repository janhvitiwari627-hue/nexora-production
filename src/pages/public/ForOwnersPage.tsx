...
const FEATURES = [
  {
    ...
    body: "AI-optimised calendar that minimises gaps and maximises stylist utilisation.",
  },
  {
    icon: Users,
    title: "Staff & rosters",
    body: "Manage schedules, leave, payroll & commissions in one place.",
  },
  {
    icon: BarChart3,
    title: "Real-time analytics",
    body: "Track revenue, repeat rate and service mix with live dashboards.",
  },
  {
    icon: MessageSquare,
    title: "WhatsApp marketing",
    body: "Automated reminders, win-back campaigns and review requests.",
  },
  {
    icon: Sparkles,
    title: "Loyalty & memberships",
    body: "Plug-and-play tiers, points and gift cards — no setup required.",
  },
  {
    icon: ShieldCheck,
    title: "Secure payments",
    body: "UPI, card and QR with auto-reconciliation and GST-ready invoices.",
  },
];
...
const PLANS = [
  {
    name: "Free",
    price: 0,
    body: "Try Nexora at zero cost.",
    perks: ["1 staff account", "Up to 20 bookings/mo", "Booking website", "Community support"],
  },
  {
    name: "Starter",
    price: 999,
    body: "For new salons getting started.",
    perks: ["Up to 3 staff", "Bookings & reminders", "Basic analytics", "Customer support"],
  },
  {
    name: "Growth",
    price: 2499,
    popular: true,
    body: "Most loved by growing salons.",
    perks: [
      "Unlimited staff",
      "WhatsApp marketing",
      "Memberships & loyalty",
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden bg-[#0A2540] py-24 md:py-32">
        ...
        <div className="relative mx-auto max-w-6xl px-4 text-center md:px-6">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-base text-white/80 md:text-lg">
            Bookings, payments, marketing, loyalty and analytics — beautifully unified. Built with
            the polish of Stripe and the reach of Airbnb.
          </p>
      ...
      <section className="mx-auto max-w-4xl px-4 py-20 text-center md:px-6">
        <h2 className="text-heading text-3xl font-black md:text-5xl">
          Running a salon is hard. Software shouldn't be.
        </h2>
        <p className="text-muted-foreground mx-auto mt-5 max-w-2xl text-base md:text-lg">
          Juggling WhatsApp DMs, paper diaries, and Excel for staff payouts? Nexora replaces all of
          it with one elegant platform built around the way you actually work.
        </p>
      </section>
        ...
        <div className="mt-5 flex justify-center gap-2">
          {SHOTS.map((_, i) => (
            <button
              key={i}
              onClick={() => setShot(i)}
              className={`h-2 rounded-full transition-all ${i === shot ? "bg-primary w-10" : "bg-muted w-2"}`}
            />
          ))}
        </div>
      </section>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {PLANS.map((p) => (
            ...
            >
              ...
              <div className="text-heading mt-5 text-3xl font-black sm:text-4xl">
              ...
              <button
                className={`mt-6 w-full rounded-[var(--radius-button)] px-4 py-3 text-sm font-bold ${p.popular ? "bg-gradient-cta text-primary-foreground shadow-[var(--shadow-glow)]" : "border-primary text-primary border"}`}
              >
                Choose {p.name}
              </button>
            </article>
        ...
        <div className="mx-auto max-w-5xl px-4 md:px-6">
          <div className="grid gap-4 md:grid-cols-2">
            {[
              {
                name: "Aanya Sharma",
                salon: "Looks Unisex, Jaipur",
                text: "Bookings went up 42% in 3 months. The WhatsApp marketing alone paid for itself in a week.",
              },
              {
                name: "Vikram Singh",
                salon: "Studio Noir, Mumbai",
                text: "I finally have one screen that tells me exactly how my business is doing. Game-changer.",
              },
            ].map((t) => (
              <article
                key={t.name}
