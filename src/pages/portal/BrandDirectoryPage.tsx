...
export function BrandDirectoryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return brands.filter((b) =>
      [b.name, b.tagline, b.description, b.category, b.hq_city, b.hq_state].some((x) =>
    ...
    <PortalLayout>
        ...
        action={
          <Button asChild className="bg-gradient-cta text-primary-foreground">
            <Link to="/portal/brands/register">
              <Plus className="mr-1 h-4 w-4" /> Register Brand
            </Link>
          </Button>
        }
      />
      ...
      <div className="mb-6 flex items-center gap-2 rounded-[var(--radius-button)] border border-border/60 bg-card px-3">
        ...
        <Input
          ...
          placeholder="Search brands, categories or cities"
      ...
      {loading ? (
        ...
        <EmptyHint
          ...
          action={
            <Button asChild>
              <Link to="/portal/brands/register">Register your brand</Link>
            </Button>
          }
        />
      ) : (
              ...
              <div className="relative h-28 bg-gradient-mesh">
                ...
                {b.is_sponsored && (
                  <Badge className="absolute right-2 top-2 bg-primary text-primary-foreground">
                    <Star className="mr-1 h-3 w-3" /> Sponsored
                  </Badge>
                )}
              </div>
              <div className="p-5">
                <div className="flex items-start gap-3">
                  <div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl bg-muted">
                    {b.logo_url ? (
                      ...
                      <span className="text-base font-bold text-muted-foreground">{b.name[0]}</span>
                    )}
                  </div>
                ...
                <div className="mt-4 flex gap-2">
                  <Button asChild size="sm" variant="outline">
                    <Link to="/portal/brands/$slug" params={{ slug: b.slug }}>
                      View Profile
                    </Link>
                  </Button>
                  <LeadDialog
                    ...
                    target={{ type: "brand", id: b.id, name: b.name }}
                  />
                </div>
              </div>
            </article>
