...
export function BrandRegistrationPage() {
  ...
  const [form, setForm] = useState({
    name: "", company_name: "", owner_name: "", tagline: "", description: "", category: "",
    website: "", email: "", phone: "",
    gst_number: "", pan_number: "", business_type: "",
    address: "", hq_city: "", hq_state: "", pincode: "",
    logo_url: "", cover_url: "",
      ...
      <Card className="p-6 md:p-8">
        <div className="grid gap-5">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Field id="cn" label="Company Name" value={form.company_name} onChange={(v) => setForm({ ...form, company_name: v })} />
            <Field id="name" label="Brand Name *" value={form.name} onChange={(v) => setForm({ ...form, name: v })} />
            <Field id="owner" label="Owner Name" value={form.owner_name} onChange={(v) => setForm({ ...form, owner_name: v })} />
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <Field id="phone" label="Mobile" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} />
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <Field id="gst" label="GST Number" value={form.gst_number} onChange={(v) => setForm({ ...form, gst_number: v })} />
            <Field id="pan" label="PAN" value={form.pan_number} onChange={(v) => setForm({ ...form, pan_number: v })} />
            <div>
          ...
          <Field id="addr" label="Address" value={form.address} onChange={(v) => setForm({ ...form, address: v })} />
          <div className="grid gap-3 md:grid-cols-3">
            <Field id="state" label="State" value={form.hq_state} onChange={(v) => setForm({ ...form, hq_state: v })} />
            <Field id="city" label="City" value={form.hq_city} onChange={(v) => setForm({ ...form, hq_city: v })} />
            <Field id="pin" label="Pincode" value={form.pincode} onChange={(v) => setForm({ ...form, pincode: v })} />
          </div>

          <SectionTitle>Media & Documents</SectionTitle>
          <div className="grid gap-4 md:grid-cols-2">
            <SingleFileUpload label="Logo" value={form.logo_url} userId={user?.id} folder="brand-logos"
              onChange={(url) => setForm({ ...form, logo_url: url })} />
            <SingleFileUpload label="Cover Image" value={form.cover_url} userId={user?.id} folder="brand-covers"
              onChange={(url) => setForm({ ...form, cover_url: url })} />
          </div>
          <MultiFileUpload label="Gallery Images" values={form.gallery_urls} userId={user?.id}
            folder="brand-gallery" onChange={(v) => setForm({ ...form, gallery_urls: v })} />
          <MultiFileUpload label="Documents (GST, PAN, certificates — PDF or image)" values={form.document_urls}
            userId={user?.id} folder="brand-docs" onChange={(v) => setForm({ ...form, document_urls: v })}
