...
export function DistributorRegistrationPage() {
  ...
  const [form, setForm] = useState({
    ...
    pincode: "",
    address: "",
    coverage_states_text: "",
    coverage_districts_text: "",
    categories_text: "",
    brands_handled_text: "",
    logo_url: "",
    cover_url: "",
    ...
    getMyDistributor(user.id).then((d) => {
      if (d) {
        ...
        setForm((f) => ({
          ...f,
          ...(Object.fromEntries(
            Object.entries(d).filter(([_, v]) => v !== null && typeof v !== "object"),
          ) as any),
        ...
        <div className="grid gap-5">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <F
              ...
              onChange={(v) => setForm({ ...form, years_in_business: v })}
            />
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <F
              id="cov"
              label="Coverage States (comma separated)"
              value={form.coverage_states_text}
              onChange={(v) => setForm({ ...form, coverage_states_text: v })}
            />
            <F
              id="covd"
              label="Coverage Districts (comma separated)"
              value={form.coverage_districts_text}
              onChange={(v) => setForm({ ...form, coverage_districts_text: v })}
            />
            <F
              id="cat"
              label="Categories Handled (comma separated)"
              value={form.categories_text}
              onChange={(v) => setForm({ ...form, categories_text: v })}
            />
            <F
              id="brh"
              label="Brands Handled (comma separated)"
              value={form.brands_handled_text}
              onChange={(v) => setForm({ ...form, brands_handled_text: v })}
            />
          </div>
          ...
          />
          <MultiFileUpload
            label="Business Documents (GST, trade licence, PAN — PDF or image)"
