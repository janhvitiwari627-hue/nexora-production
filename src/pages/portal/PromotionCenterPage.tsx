    ...
    (async () => {
      ...
      if (b) {
        const [{ data: prods }, { data: promos }] = await Promise.all([
          supabase
            .from("brand_products")
            .select("id,brand_id,name,description,category,image_url,price,mrp,is_featured")
            .eq("brand_id", b.id)
            .order("name"),
          (supabase as any)
            .from("promotions")
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, product_id: v === "all" ? "" : v })}
              >
                <SelectTrigger>
                  <SelectValue placeholder="All products / brand-wide" />
                </SelectTrigger>
                <SelectContent>
              ...
              {products.length === 0 && (
                <p className="mt-1 text-xs text-muted-foreground">
                  No products yet — add some on your brand page.
                </p>
              )}
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <div>
                ...
                <Select
                  value={form.target_category || "any"}
                  onValueChange={(v) => setForm({ ...form, target_category: v === "any" ? "" : v })}
                >
                  <SelectTrigger>
            ...
            <div>
              <SingleFileUpload
                ...
                userId={user.id}
                folder={`promotions/${brand.id}`}
              />
              <p className="mt-1 text-xs text-muted-foreground">
                Recommended 1200×400, under 2 MB.
              </p>
            </div>
            ...
            <Button
              type="submit"
              disabled={submitting}
              className="w-full bg-gradient-cta text-primary-foreground"
            >
        ...
        <div>
          ...
          {promotions.length === 0 ? (
            <EmptyHint
              ...
              body="Submit your first campaign and reach thousands of salons and beauty retailers."
            />
          ) : (
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        {p.description && (
                          <p className="mt-1 text-sm text-body line-clamp-2">{p.description}</p>
                        )}
                      </div>
                      <Badge
                        variant={p.status === "active" ? "default" : "secondary"}
                        className={p.status === "active" ? "bg-emerald-500 text-white" : ""}
                      >
                        {p.status}
                      </Badge>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-muted-foreground">
                      ...
                      {p.target_state && (
                        <Badge variant="outline">
                          {[p.target_district, p.target_state].filter(Boolean).join(", ")}
                        </Badge>
                      )}
                      {p.budget != null && (
                        <Badge variant="outline">₹{Number(p.budget).toLocaleString()}</Badge>
                      )}
                    </div>
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className="text-muted-foreground">
                        {p.impressions.toLocaleString()} views · {p.clicks.toLocaleString()} clicks
                      </span>
                      <Button
                        size="sm"
                        variant="ghost"
