    ...
    <PortalLayout>
      ...
      {loading ? (
        ...
        <EmptyHint
          ...
          action={<Button asChild><Link to="/portal/distributors/register">Register as Distributor</Link></Button>}
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((d) => (
            <article key={d.id} className="rounded-[var(--radius-card)] border border-border/60 bg-card p-5 transition hover:shadow-[var(--shadow-card)]">
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-3">
                  <div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl bg-muted">
                    {d.logo_url ? <img src={d.logo_url} alt="" className="h-full w-full object-cover" /> : <span className="font-bold text-muted-foreground">{d.company_name[0]}</span>}
                  </div>
                ...
                {d.is_sponsored && <Badge className="bg-primary text-primary-foreground"><Star className="mr-1 h-3 w-3" /> Featured</Badge>}
              </div>

              <div className="mt-3 flex flex-wrap gap-1.5">
                {(d.categories ?? []).slice(0, 3).map((c) => <Badge key={c} variant="secondary">{c}</Badge>)}
              </div>
