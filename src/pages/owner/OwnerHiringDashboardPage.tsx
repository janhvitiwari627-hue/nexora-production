...
export function OwnerHiringDashboardPage() {
  ...
  const move = (id: string, to: Applicant["stage"]) =>
    setApplicants((p) => p.map((a) => (a.id === id ? { ...a, stage: to } : a)));
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Manage applicants across all your job postings
          </p>
        </div>
        <Button asChild>
          <a href="/owner/jobs/new">
        ...
        <CardContent className="grid gap-3 md:grid-cols-3">
          {MOCK_JOBS.slice(0, 6).map((j) => (
            <button
              ...
              className={`rounded-lg border p-4 text-left transition-colors ${activeJob === j.id ? "border-primary bg-primary/5" : "hover:bg-muted/50"}`}
            >
      ...
      <Card>
        <CardHeader>
          <CardTitle>Pipeline — {MOCK_JOBS.find((j) => j.id === activeJob)?.title}</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={!!scheduleFor} onOpenChange={(o) => !o && setScheduleFor(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Schedule Interview — {scheduleFor?.name}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                if (scheduleFor) move(scheduleFor.id, "Interview");
                setScheduleFor(null);
