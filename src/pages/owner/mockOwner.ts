...
export const revenueDaily = Array.from({ length: 14 }, (_, i) => ({
  label: `D${i + 1}`,
  revenue: 8000 + Math.round(Math.sin(i / 2) * 3000 + Math.random() * 4000 + i * 400),
}));
export const revenueWeekly = Array.from({ length: 8 }, (_, i) => ({
  label: `W${i + 1}`,
  revenue: 45000 + Math.round(Math.cos(i / 2) * 8000 + Math.random() * 6000 + i * 1500),
}));
export const revenueMonthly = [
  "Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"
].map((m, i) => ({ label: m, revenue: 180000 + Math.round(Math.sin(i) * 20000 + Math.random() * 30000 + i * 4000) }));

// month calendar density (current month)
...
export const calendarFirstWeekday = new Date(today.getFullYear(), today.getMonth(), 1).getDay();
export const calendarMonthLabel = today.toLocaleString("en-US", { month: "long", year: "numeric" });
