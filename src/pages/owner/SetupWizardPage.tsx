...
function sanitizeSetupPatch(patch: Partial<Form>, options: { omitIncompleteUpi?: boolean } = {}) {
...
function friendlySetupError(error: unknown) {
  const message =
    error instanceof Error ? error.message : String(error || "Please check the setup fields.");
...
export function SetupWizardPage() {
  ...
  const { activeSalonId, isLoading: ownerLoading, hasSalon } = useOwnerContext();
  ...
  const goToStep = (i: number) => {
    ...
    if (stepStorageKey)
      try {
        localStorage.setItem(stepStorageKey, String(i));
      } catch {
        /* noop */
      }
  };
  ...
  const [form, setForm] = useState<Form>({
    ...
    home_service_charge: 0,
    home_service_radius_km: 0,
  });
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      is_home_service: s.is_home_service ?? false,
      home_service_charge: Number(s.home_service_charge ?? 0),
      home_service_radius_km: Number(s.home_service_radius_km ?? 0),
    }));
  ...
  const checklist = useMemo(() => {
    ...
    return [
      ...
      { id: "addr", label: "Address", done: form.address.trim().length >= 5 },
      {
        ...
        done: form.latitude != null && form.longitude != null,
      },
      ...
      { id: "cover", label: "Cover banner", done: !!form.cover_image_url },
      {
        ...
        done: services.some((s) => s.name.trim() && s.price > 0),
      },
      {
        id: "hours",
        label: "Working hours",
        done: Object.values(form.hours).every((h) => h.closed || (!!h.open && !!h.close)),
      },
      { id: "upi", label: "Nexora QR (UPI)", done: validUpi },
      {
        id: "home",
        label: "Home-service settings",
        done: !form.is_home_service || form.home_service_radius_km > 0,
      },
    ];
  }, [form, services]);
  ...
  const goLive = useMutation({
    ...
    onSuccess: (r) => {
      if (!r.ok) {
        toast.error(
          `Still missing: ${r.missing.slice(0, 3).join(", ")}${r.missing.length > 3 ? "…" : ""}`,
        );
        return;
      }
      ...
      toast.success(
        r.awaitingApproval
          ? "Setup saved. Your salon will go live after admin approval."
        ...
        await updateFn({
          data: {
            ...
            patch: sanitizeSetupPatch({
              ...
              home_service_charge: form.home_service_charge,
              home_service_radius_km: form.home_service_radius_km,
            }),
          },
        });
  ...
  const uploadImage = async (file: File, kind: "logo" | "cover") => {
    ...
    if (!file.type.startsWith("image/") || file.size > 2 * 1024 * 1024) {
      toast.error("Please upload an image of 2 MB or less.");
      return null;
    }
    ...
    try {
      ...
      setAutosave({ status: "saving", at: null, error: null });
      const patch =
        kind === "logo" ? { logo_url: result.secure_url } : { cover_image_url: result.secure_url };
    ...
    return (
      <div className="container mx-auto max-w-2xl py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          You need a registered salon before launching your website.
        </p>
          ...
          <Card className="p-6">
                ...
                <Grid2>
                  <Field label="Short description">
                    <Textarea
                      ...
                      placeholder="A short introduction customers will see first"
                    />
                  </Field>
                  <Field label="About your salon">
                    <Textarea
                      ...
                      placeholder="Your experience, specialities and what makes the salon trusted"
                    />
                  </Field>
                </Grid2>
            ...
            {step.id === "location" && (
              <div className="space-y-5">
                <SectionHeader
                  ...
                  subtitle="Customers tap the pin for directions."
                />
                ...
                <StepActions
                  onSave={() =>
                    saveStep.mutate({ latitude: form.latitude, longitude: form.longitude })
                  }
                  saving={saveStep.isPending}
                  next={() => setStepIdx(stepIdx + 1)}
            ...
            {step.id === "brand" && (
              <div className="space-y-5">
                <SectionHeader
                  ...
                  subtitle="Square logo + wide cover work best."
                />
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <ImagePicker
                      ...
                      onPick={async (f) => {
                        const u = await uploadImage(f, "logo");
                        if (u) setForm({ ...form, logo_url: u });
                      }}
                    />
                  ...
                  <div className="space-y-1.5">
                    <ImagePicker
                      ...
                      aspect="aspect-[16/9]"
                      onPick={async (f) => {
                        const u = await uploadImage(f, "cover");
                        if (u) setForm({ ...form, cover_image_url: u });
                      }}
                    />
            ...
            {step.id === "hours" && (
              <div className="space-y-5">
                <SectionHeader
                  icon={Clock}
                  title="Working hours"
                  subtitle="Bookings only accept inside these slots."
                />
                <div className="space-y-2">
                  {DAYS.map((d) => {
                    ...
                    return (
                      <div
                        key={d.key}
                        className="grid grid-cols-[80px_1fr_1fr_120px] items-center gap-3"
                      >
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              hours: { ...form.hours, [d.key]: { ...h, open: e.target.value } },
                            })
                          }
                        />
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              hours: { ...form.hours, [d.key]: { ...h, close: e.target.value } },
                            })
                          }
                        />
                        <label className="inline-flex items-center gap-2 text-sm">
                          <Checkbox
                            ...
                            onCheckedChange={(c) =>
                              setForm({
                                ...form,
                                hours: { ...form.hours, [d.key]: { ...h, closed: !!c } },
                              })
                            }
                          />
                          Closed
                  ...
                  <label className="flex items-center justify-between gap-4">
                    <span>
                      ...
                      <span className="text-xs text-muted-foreground">
                        Customers can book service at their address.
                      </span>
                    </span>
                    <Checkbox
                      checked={form.is_home_service}
                      onCheckedChange={(checked) =>
                        setForm({ ...form, is_home_service: !!checked })
                      }
                    />
                  </label>
                    ...
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      <Field label="Extra charge (₹)">
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({ ...form, home_service_charge: Number(e.target.value) || 0 })
                          }
                        />
                      </Field>
                      <Field label="Service radius (km) *">
                        <Input
                          ...
                          min={1}
                          max={100}
                          value={form.home_service_radius_km || ""}
                          onChange={(e) =>
                            setForm({
                              ...form,
                              home_service_radius_km: Number(e.target.value) || 0,
                            })
                          }
                        />
                      </Field>
            ...
            {step.id === "services" && (
              <div className="space-y-5">
                <SectionHeader
                  ...
                  subtitle="Add at least one service. You can add more later."
                />
                  ...
                  {services.map((s, i) => (
                    <div key={i} className="grid grid-cols-[1fr_120px_120px] gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const c = [...services];
                          c[i] = { ...c[i], price: Number(e.target.value) || 0 };
                          setServices(c);
                        }}
                      />
                      <Input
                        ...
                        onChange={(e) => {
                          const c = [...services];
                          c[i] = { ...c[i], duration_minutes: Number(e.target.value) || 30 };
                          setServices(c);
                        }}
                      />
                    </div>
            ...
            {step.id === "payment" && (
              <div className="space-y-5">
                <SectionHeader
                  icon={QrCode}
                  title="Nexora QR — UPI setup"
                  subtitle="Bookings accept advance via your UPI. Settlement runs daily at 10 PM."
                />
                ...
                <p className="text-xs text-muted-foreground">
                  Only Nexora QR (UPI) is supported in V1. Platform commission applies only on
                  successful transactions — no monthly SaaS fee.
                </p>
                <StepActions
            ...
            {step.id === "review" && (
              <div className="space-y-5">
                <SectionHeader
                  ...
                  subtitle="Everything below must be green before we publish."
                />
                <div className="grid gap-2 sm:grid-cols-2">
                  {checklist.map((c) => (
                    <div
                      key={c.id}
                      className="flex items-center gap-2 rounded-md border bg-card/50 px-3 py-2 text-sm"
                    >
                      <span
                        className={`grid h-5 w-5 place-items-center rounded-full ${c.done ? "bg-emerald-600 text-white" : "bg-muted text-muted-foreground"}`}
                      >
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            ...
            <ul className="space-y-1.5">
              {checklist.map((c) => (
                <li key={c.id} className="flex items-center gap-2 text-sm">
                  <span
                    className={`grid h-4 w-4 place-items-center rounded-full ${c.done ? "bg-emerald-600 text-white" : "border bg-background text-muted-foreground"}`}
                  >
                    {c.done && <Check className="h-2.5 w-2.5" />}
                  </span>
                  <span className={c.done ? "text-heading" : "text-muted-foreground"}>
                    {c.label}
                  </span>
                </li>
              ))}
          ...
          <Card className="bg-primary/5 p-5 text-sm">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Fill these 12 items and tap <b>Go live</b> — your white-label site, booking engine,
              and Nexora QR start working immediately.
            </p>
          </Card>
        </aside>
      </div>
...
function ImagePicker({
  ...
  return (
    <div>
      ...
      <label
        className={`relative grid ${aspect} cursor-pointer place-items-center overflow-hidden rounded-xl border-2 border-dashed border-border bg-muted/40 hover:border-primary`}
      >
        {url ? (
...
function AutosaveBadge({
  ...
  state: { status: "idle" | "saving" | "saved" | "error"; at: number | null; error: string | null };
}) {
  const time = state.at
  ...
  if (state.status === "saving") {
    ...
    label = "Save failed — see details above";
