...
export function OwnerOnboardingPage() {
  ...
  const draftKey = useMemo(
    () => (authUser ? `${DRAFT_KEY_PREFIX}${authUser.id}` : ANON_DRAFT_KEY),
    [authUser],
  );
  ...
  useEffect(() => {
    ...
    if (d) {
      ...
      if (anonDraft && authUser) {
        try {
          window.localStorage.removeItem(ANON_DRAFT_KEY);
        } catch {
          /* noop */
        }
      }
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <header className="text-center">
          ...
          <p className="text-muted-foreground text-sm">
            Get your white-label website live in 7 quick steps
          </p>
              ...
              >
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <Field label="Mobile">
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setOtpSent(true);
                              toast.success("OTP sent (WhatsApp verification in Phase 7)");
                            }}
                          >
                      ...
                      {otpSent && (
                        <Field label="OTP">
                          <Input
                            maxLength={6}
                            value={account.otp}
                            onChange={(e) => setAccount({ ...account, otp: e.target.value })}
                            placeholder="6-digit"
                          />
                        </Field>
                      )}
                      ...
                      <Field label="Password">
                        <Input
                          ...
                          disabled={accountCreated}
                          placeholder={accountCreated ? "Account already created" : ""}
                        />
                        {!accountCreated && (
                      ...
                      <Field label="Business Category">
                        <Select
                          ...
                          onValueChange={(v) => setAccount({ ...account, category: v })}
                        >
                          <SelectTrigger>
                  ...
                  <Step title="Business Details" subtitle="Where customers will find you">
                    <div className="grid gap-3 md:grid-cols-2">
                      ...
                      <Field label="Business Type">
                        <Select
                          value={business.type}
                          onValueChange={(v) => setBusiness({ ...business, type: v })}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <Field label="Pin your location" className="md:col-span-2">
                        ...
                        <div className="text-muted-foreground text-xs">
                          Lat {business.lat.toFixed(4)} · Lng {business.lng.toFixed(4)}
                        </div>
                      </Field>
                ...
                {step === 2 && (
                  <Step
                    ...
                    subtitle={
                      <>
                        Add at least <strong>5 services</strong> to publish
                      </>
                    }
                  >
                    <div className="space-y-3">
                      {services.map((s, idx) => (
                        <div
                          key={idx}
                          className="bg-muted/30 grid items-start gap-2 rounded-lg border p-3 md:grid-cols-[2fr_1fr_1fr_auto]"
                        >
                          <Input
                                ...
                                setServices((p) =>
                                  p.map((x, i) =>
                                    i === idx
                                      ? {
                                          ...x,
                                          desc: `Premium ${x.name || "service"} crafted by skilled professionals using top brands.`,
                                        }
                                      : x,
                                  ),
                                );
                      ...
                      <Badge variant={services.length >= 5 ? "default" : "destructive"}>
                        {services.length}/5 minimum
                      </Badge>
                    </div>
                  </Step>
                )}
                  ...
                  <Step title="Staff" subtitle="Add your team members">
                    <div className="space-y-3">
                      {staff.map((s, idx) => (
                        <div
                          key={idx}
                          className="bg-muted/30 grid items-center gap-2 rounded-lg border p-3 md:grid-cols-[80px_2fr_1.5fr_1fr_auto]"
                        >
                          <label className="bg-muted grid h-16 w-16 cursor-pointer place-items-center rounded-full text-xs">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setStaff((p) =>
                                p.map((x, i) =>
                                  i === idx ? { ...x, designation: e.target.value } : x,
                                ),
                              )
                            }
                          />
                          <Input
                            ...
                            onChange={(e) =>
                              setStaff((p) =>
                                p.map((x, i) =>
                                  i === idx ? { ...x, experience: e.target.value } : x,
                                ),
                              )
                            }
                          />
                          <Button
                            ...
                            onClick={() => setStaff((p) => p.filter((_, i) => i !== idx))}
                          >
                            <Trash2 className="text-destructive h-4 w-4" />
                          </Button>
                        </div>
                      ...
                      <Button
                        variant="outline"
                        onClick={() =>
                          setStaff((p) => [...p, { name: "", designation: "", experience: "" }])
                        }
                      >
                    ...
                    {gallery.length > 0 && (
                      <div className="mt-3 grid grid-cols-3 gap-2 md:grid-cols-5">
                        {gallery.map((g, i) => (
                          <div
                            key={i}
                            className="bg-muted grid aspect-square place-items-center rounded-lg text-center text-xs"
                          >
                            {g}
                          </div>
                        ))}
                      ...
                      <Field label="UPI QR Code">
                        <label className="bg-muted/30 hover:bg-muted grid cursor-pointer place-items-center rounded-xl border-2 border-dashed py-10">
                          ...
                          <input
                            ...
                            hidden
                            onChange={(e) =>
                              setPayment({ ...payment, qrName: e.target.files?.[0]?.name || "" })
                            }
                          />
                        </label>
                      </Field>
                ...
                {step === 6 && (
                  <Step title="Review & Publish" subtitle="One last check before going live">
                    <Accordion type="single" collapsible defaultValue="acc">
                      <AccordionItem value="acc">
                        ...
                        <AccordionContent className="text-sm">
                          {account.ownerName} · {account.businessName} · {account.email}
                        </AccordionContent>
                      </AccordionItem>
                      <AccordionItem value="biz">
                      ...
                      <AccordionItem value="srv">
                        <AccordionTrigger>Services ({services.length})</AccordionTrigger>
                        <AccordionContent className="text-sm">
                          {services
                            .map((s) => s.name)
                            .filter(Boolean)
                      ...
                      <AccordionItem value="stf">
                        <AccordionTrigger>Staff ({staff.length})</AccordionTrigger>
                        <AccordionContent className="text-sm">
                          {staff
                            .map((s) => s.name)
                            .filter(Boolean)
                      ...
                      <AccordionItem value="gal">
                        <AccordionTrigger>Gallery ({gallery.length} photos)</AccordionTrigger>
                        <AccordionContent className="text-sm">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button
                        variant="outline"
                        onClick={() => toast.success("AI content generated for all sections")}
                      >
    ...
    <div className="flex items-center justify-between">
      {STEPS.map((label, i) => (
        <div key={label} className="flex flex-1 items-center">
          <div
            className={`grid h-8 w-8 shrink-0 place-items-center rounded-full text-xs font-bold transition-colors ${i < current ? "bg-emerald-500 text-white" : i === current ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
    ...
    <div className="mt-1 space-y-1">
      <div className="flex gap-1">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded ${i < score ? colors[score - 1] : "bg-muted"}`}
          />
        ))}
      </div>
    ...
    <div className="bg-gradient-to-b from-emerald-50 to-white min-h-screen p-8">
      <div className="mx-auto max-w-md text-center">
        <motion.div
          ...
          className="mx-auto mb-6 grid h-24 w-24 place-items-center rounded-full bg-emerald-500 text-white shadow-xl"
        >
        ...
        <Card className="mt-6 text-left">
          <CardContent className="space-y-3 p-5">
            <div>
              ...
              <a href={url} className="text-primary font-mono text-sm break-all">
                {url}
              </a>
            </div>
            <div className="bg-muted grid h-32 w-32 place-items-center rounded-lg mx-auto text-xs">
              QR Code
            </div>
            <Button
              className="w-full"
              onClick={() =>
                navigator
                  .share?.({ url })
                  .catch(() => toast.success("Share dialog not supported, link copied"))
              }
            >
