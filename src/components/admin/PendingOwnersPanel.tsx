...
export function PendingOwnersPanel() {
  ...
  const mut = useMutation({
    mutationFn: ({ id, approve }: { id: string; approve: boolean }) =>
      decide({ data: { owner_request_id: id, approve } }),
    onSuccess: (_d, vars) => {
      ...
      <CardContent className="p-0">
        <div className="flex items-center justify-between border-b px-4 py-3">
          <div className="flex items-center gap-2">
            <Badge className="bg-warning/10 text-warning border-0">{items.length} pending</Badge>
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {r.salon?.city && (
                      <span className="text-xs text-muted-foreground">· {r.salon.city}</span>
                    )}
                  </div>
