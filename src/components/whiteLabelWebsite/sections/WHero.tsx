...
export function WHero({ shop, template }: { shop: ShopData; template: TemplateConfig }) {
  ...
  if (template.hero === "split") {
    return (
      <section
        className={`grid items-center gap-8 px-6 py-16 md:grid-cols-2 md:px-12 md:py-24 ${animClass}`}
      >
  ...
  if (template.hero === "beautyBanner") {
    return (
      <section
        className={`relative overflow-hidden px-6 py-16 md:px-12 md:py-24 ${animClass}`}
        style={{ backgroundColor: template.colors.bg, color: template.colors.text }}
      >
        <div className="mx-auto grid max-w-7xl items-center gap-10 md:grid-cols-[0.9fr_1.1fr]">
          <div className="blossom-bloom relative z-10 space-y-5">
            ...
            <h1
              className="max-w-xl text-5xl font-semibold leading-tight md:text-7xl"
              style={{ fontFamily: template.headingFont }}
            >
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span>
                <strong>{shop.rating}</strong> from {shop.reviewCount} beauty clients
              </span>
            </div>
          </div>
          <div className="blossom-float relative">
            <div
              className="absolute -left-6 top-10 hidden h-28 w-28 rounded-full md:block"
              style={{ backgroundColor: template.colors.secondary, opacity: 0.35 }}
            />
            <img
              src={shop.coverImage}
              alt={shop.name}
            ...
            />
            <div
              className="absolute -bottom-6 right-6 max-w-xs rounded-3xl p-5 shadow-xl"
  ...
  return (
    <section
      className={`relative grid h-[90vh] min-h-[640px] place-items-center overflow-hidden text-white ${animClass}`}
    >
      <video
        autoPlay
        muted
      ...
      >
        <source
          src={
            shop.videoUrl ??
            "https://cdn.coverr.co/videos/coverr-luxury-spa-treatment-2417/1080p.mp4"
          }
          type="video/mp4"
        />
      </video>
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at 50% 60%, rgba(212,175,55,0.25), transparent 55%)",
        }}
      />
      <div
      ...
      <div className="royal-rise relative z-10 max-w-3xl space-y-6 px-6 text-center">
        <div
          className="flex items-center justify-center gap-3 text-[10px] font-semibold uppercase tracking-[0.5em]"
        ...
        <h1
          className="text-5xl font-light leading-[1.05] md:text-7xl lg:text-8xl"
          style={{ fontFamily: template.headingFont }}
        >
        ...
        />
        <p className="mx-auto max-w-xl text-base font-light tracking-wide opacity-85 md:text-lg">
          {shop.tagline}
        </p>
        <div className="flex flex-wrap justify-center gap-3 pt-4">
          <Button
            size="lg"
            className="royal-shimmer relative overflow-hidden border-0 px-8 py-6 text-sm font-medium uppercase tracking-[0.25em]"
            style={{
          ...
          >
            <Link
              ...
              search={{ service: undefined }}
            >
              <Calendar className="h-4 w-4" /> Reserve Your Visit
            </Link>
          </Button>
          <Button
            size="lg"
            variant="outline"
            className="border px-8 py-6 text-sm font-medium uppercase tracking-[0.25em] text-white hover:bg-white/10"
            style={{
        ...
        <div className="flex items-center justify-center gap-2 pt-6 text-xs opacity-75">
          ...
          <span className="font-medium tracking-widest">
            {shop.rating} · {shop.reviewCount} GUEST REVIEWS
          </span>
        </div>
      </div>
