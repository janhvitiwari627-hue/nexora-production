...
export function WReferral({ shop, template }: { shop: ShopData; template: TemplateConfig }) {
  const referralCode = useMemo(() => `${shop.slug.toUpperCase().replace(/-/g, "").slice(0, 6)}-${Math.floor(1000 + Math.random() * 9000)}`, [shop.slug]);
    ...
    <section id="referral" className="px-6 py-16 md:px-12">
      ...
      >
        <div className="space-y-3">
          <div>
            <label className="text-muted-foreground text-xs uppercase tracking-wider">Your Referral Code</label>
            <div className="mt-1 flex items-center gap-2 rounded-md border bg-white px-3 py-2">
              ...
              <button onClick={() => copy(referralCode, "Code")} aria-label="Copy code" className="text-muted-foreground hover:text-foreground">
                <Copy className="h-4 w-4" />
              </button>
            </div>
          </div>
          <div>
            <label className="text-muted-foreground text-xs uppercase tracking-wider">Your Referral Link</label>
            <div className="mt-1 flex items-center gap-2 rounded-md border bg-white px-3 py-2">
              <span className="flex-1 truncate font-mono text-xs">{referralLink}</span>
              <button onClick={() => copy(referralLink, "Link")} aria-label="Copy link" className="text-muted-foreground hover:text-foreground">
                <Copy className="h-4 w-4" />
              </button>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <ShareButton title={`Join me at ${shop.name}`} text={`Use my code ${referralCode}`} url={referralLink} label="Share Link" />
            <button
              type="button"
        ...
        <div className="grid place-items-center">
          {showQR ? (
            <img src={qrUrl} alt="Referral QR" className="h-44 w-44 rounded-md border bg-white p-2" />
          ) : (
