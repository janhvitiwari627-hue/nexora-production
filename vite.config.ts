...
export default defineConfig(((env: { command: "serve" | "build" }) => ({
  tanstackStart: {
  ...
  vite: {
    ...
    // gated to `command === 'serve'`. This keeps the production build
    // identical to before the fix (no risk of breaking the Lovable preview
    // pipeline) while still unblocking dev-mode hydration.
    optimizeDeps:
      env.command === "serve"
        ? {
            include: [
